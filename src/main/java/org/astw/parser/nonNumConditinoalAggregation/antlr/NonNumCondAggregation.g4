...
parseConditionalAggregation returns [NonNumConditionalAggregation parsedNonNumConditionalAggregation]: 
        nac=nonNumAggContent EOF{
                                
            $parsedNonNumConditionalAggregation = $nac.nonNumConditionalAggregation;
        };

nonNumAggContent returns [NonNumConditionalAggregation nonNumConditionalAggregation]: 
        ...
        aggCond=aggCondition{
            ...
            try{
                $nonNumConditionalAggregation = 
                    new NonNumConditionalAggregation($aggSrc.aggregationSource,
...
aggSource returns [NonNumExp aggregationSource]: 
        nneString=String{
            ...
            System.out.println("DEBUGA: AggSrc-String: "+$nneString.text+" -> "+str);
            $aggregationSource = new NonNumExpString(str);
         }
         | nneBool=(True | False){
            System.out.println("DEBUGA: AggSrc-True: "+$nneBool.text);
            try{
                $aggregationSource = new NonNumExpBoolean($nneBool.text);
            }catch(Exception e){
              notifyErrorListeners("ERROR: " + e.getMessage());
            }
         } 
         | nneQuery=query{
            System.out.println("DEBUGA: AggSrc-Query: "+$nneQuery.text);
            $aggregationSource = $nneQuery.attAccessor;
         }
         | LP nne=nonNumExp RP{
            System.out.println("DEBUGA: AggSrc-BRACKET: "+$nne.text);
            $aggregationSource = new BracketNonNumExp($nne.nonNumericExpression);
         }
         ;
//----------------------------------------------
